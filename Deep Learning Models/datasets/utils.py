...
def project2vector(text, bert_pre_tokenizer):
    ...
    input_ids = pad_sequences(input_ids, maxlen=MAX_LEN_project, dtype="long", truncating="post", padding="post")
    return input_ids
...
def honour2vector(text, bert_pre_tokenizer):
    ...
    tokenized_sents = [tokenizer.tokenize(sent) for sent in sentencses]
    input_ids = [tokenizer.convert_tokens_to_ids(sent) for sent in tokenized_sents]
    input_ids = pad_sequences(input_ids, maxlen=MAX_LEN_honour, dtype="long", truncating="post", padding="post")
    return input_ids
